...
GrpcVeloxService::GrpcVeloxService(const WorkerEnv* worker_env,
                                   ::grpc::ServerBuilder* builder)
    : is_shutdown_(false), worker_env_(worker_env) {
  builder->RegisterService(&velox_service_);
  cq_ = builder->AddCompletionQueue().release();
}

GrpcVeloxService::~GrpcVeloxService() {
  delete shutdown_alarm_;
  delete cq_;
}

void GrpcVeloxService::Shutdown() {
...
#define ENQUEUE_REQUEST(method, supports_cancel)                             \
  do {                                                                       \
    mutex_lock l(shutdown_mu_);                                              \
    if (!is_shutdown_) {                                                     \
      Call<GrpcVeloxService, grpc::VeloxService::AsyncService,               \
           method##Request, method##Response>::                              \
          EnqueueRequest(&velox_service_, cq_,                               \
                         &grpc::VeloxService::AsyncService::Request##method, \
                         &GrpcVeloxService::method##Handler,                 \
                         (supports_cancel));                                 \
    }                                                                        \
  } while (0)

// This method blocks forever handling requests from the completion queue.
void GrpcVeloxService::HandleRPCsLoop() {
  for (int i = 0; i < 10; ++i) {
  ...
  while (cq_->Next(&tag, &ok)) {
    UntypedCall<GrpcVeloxService>::Tag* callback_tag =
        static_cast<UntypedCall<GrpcVeloxService>::Tag*>(tag);
    if (callback_tag) {
...
void GrpcVeloxService::GetRemoteAddressHandler(
...
// synchronous method
Status GrpcVeloxService::GetRemoteAddressSync(
...
void SetNewVeloxService(GrpcVeloxService** handle, const WorkerEnv* worker_env,
                        ::grpc::ServerBuilder* builder) {
  *handle = new GrpcVeloxService(worker_env, builder);
}
